...
namespace Utilities
{
    ...
    {
        [SerializeField] [Header("Animation Settings")] [Tooltip("Start scale when showing")]
        private Vector3 startScale = Vector3.one * 0f;

        [SerializeField] [Tooltip("Overshoot scale")]
        private Vector3 overshootScale = Vector3.one * 1.25f;

        [SerializeField] [Tooltip("End scale after showing")]
        private Vector3 endScale = Vector3.one;

        [SerializeField] [Tooltip("Duration of show animation in seconds")]
        private float showDuration = 0.3f;

        [SerializeField] [Tooltip("Duration of hide animation in seconds")]
        private float hideDuration = 0.2f;

        [SerializeField] [Range(0f, 1f)] [Tooltip("Ease type for show animation")]
        protected float overshootRatio = 0.6f;

        [SerializeField] [Range(0f, 1f)] [Tooltip("Portion of showDuration for settling to final scale (0 to 1)")]
        protected float settleRatio = 0.4f;

        [SerializeField] [Tooltip("Ease type for show animation")]
        private Ease showEase = Ease.OutBack;

        [SerializeField] [Tooltip("Ease type for hide animation")]
